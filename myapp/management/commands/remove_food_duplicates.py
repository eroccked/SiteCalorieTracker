...
class Command(BaseCommand):
    help = 'Видаляє дублікати записів у моделі Food, залишаючи лише один, на основі назви та поживних значень.'

    def handle(self, *args, **options):
        self.stdout.write(self.style.SUCCESS('Пошук та видалення дублікатів їжі...'))

        # Знаходимо дублікати за комбінацією назви, вуглеводів, калорій, білків та жирів
        duplicate_foods_query = Food.objects.values('name', 'carbs', 'calories', 'protein', 'fats').annotate(
            item_count=Count('id')
        ...
        for item_data in duplicate_foods_query:
            ...
            self.stdout.write(
                f'Знайдено дублікати для: "{food_name}" (К:{food_carbs}, Кал:{food_calories}, Б:{food_protein}, Ж:{food_fats})')
            ...
            if foods_to_delete.exists():
                ...
                self.stdout.write(f'  Дублікатів для "{food_name}" не знайдено (можливо, вже видалено).')
